...
function createSseConnection(options: UseSseOptions = {}) {
  ...
  // 处理认证失败（401）：静默刷新 token，成功则重连，失败则保持断开
  let authRetryPending = false;

  const handleAuthFailed = async () => {
    if (authRetryPending) return;
    authRetryPending = true;
    try {
      await useUserStoreHook().refreshTokenOnce();
      // 刷新成功，重置重连状态后用新 token 重连
      resetReconnectState();
      connect();
    } catch {
      // 刷新也失败了 → 保持断开，request 拦截器会负责跳转登录页
      log("SSE token 刷新失败，停止重连");
    } finally {
  ...
  const connect = () => {
    ...
    fetch(config.url, {
      ...
      .catch((err: unknown) => {
        ...
        if (httpStatus === 401) {
          // 认证失败：不执行指数退避重连，改为触发 token 刷新
          log("SSE 收到 401，尝试刷新 token...");
          connectionState.value = SseConnectionState.DISCONNECTED;
          handleAuthFailed();
          return;
        }
