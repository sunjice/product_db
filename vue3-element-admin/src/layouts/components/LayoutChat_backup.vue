<template>
  ...
  >
    ...
    <!-- 四边 + 四角隐形缩放区域 -->
    <div class="resize-edge top" @mousedown.stop="startResize($event, 'top')" />
    <div class="resize-edge bottom" @mousedown.stop="startResize($event, 'bottom')" />
    <div class="resize-edge left" @mousedown.stop="startResize($event, 'left')" />
    <div class="resize-edge right" @mousedown.stop="startResize($event, 'right')" />
    <div class="resize-corner tl" @mousedown.stop="startResize($event, 'tl')" />
    <div class="resize-corner tr" @mousedown.stop="startResize($event, 'tr')" />
    <div class="resize-corner bl" @mousedown.stop="startResize($event, 'bl')" />
    <div class="resize-corner br" @mousedown.stop="startResize($event, 'br')" />
  </div>
</template>
  ...
  if (val.startsWith("/agent ")) {
    ...
    try {
      ...
      const resultMsg: ChatMessageType = {
        ...
        session_id: activeSessionId.value,
        role: "system",
        msg_type: result.success ? "text" : "text",
        content: summary,
...
let resizeCorner = "" as "top" | "bottom" | "left" | "right" | "tl" | "tr" | "bl" | "br"
let resizeStartX = 0
let resizeStartY = 0
let resizeStartW = 0
let resizeStartH = 0
...
function startResize(e: MouseEvent, corner: "top" | "bottom" | "left" | "right" | "tl" | "tr" | "bl" | "br") {
  e.preventDefault()
  ...
  resizeStartLeft = floatX.value
  resizeStartTop = floatY.value
...
function onResizeMove(e: MouseEvent) {
  ...
  // 约束最小/最大尺寸
  const minW = 320, maxW = 900
  const minH = 360, maxH = 800

  if (newW < minW) {
    newW = minW
    if (resizeCorner === "bl" || resizeCorner === "tl" || resizeCorner === "left") newLeft = resizeStartLeft + resizeStartW - minW
  }
  if (newW > maxW) {
    newW = maxW
    if (resizeCorner === "bl" || resizeCorner === "tl" || resizeCorner === "left") newLeft = resizeStartLeft + resizeStartW - maxW
  }
  if (newH < minH) {
    newH = minH
    if (resizeCorner === "tr" || resizeCorner === "tl" || resizeCorner === "top") newTop = resizeStartTop + resizeStartH - minH
  }
  if (newH > maxH) {
    newH = maxH
    if (resizeCorner === "tr" || resizeCorner === "tl" || resizeCorner === "top") newTop = resizeStartTop + resizeStartH - maxH
  }

  panelWidth.value = newW
  panelHeight.value = newH
  floatX.value = newLeft
  floatY.value = newTop
}
