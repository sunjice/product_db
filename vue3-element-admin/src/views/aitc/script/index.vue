    ...
    <el-card>
      ...
      <div class="flex gap-2 items-center mb-4 flex-wrap">
        ...
        <el-select v-model="queryParams.status" placeholder="状态" clearable style="width: 120px" @change="loadData">
          <el-option label="草稿" :value="ScriptStatusEnum.DRAFT" />
          <el-option label="已入库" :value="ScriptStatusEnum.PUBLISHED" />
        </el-select>
        <el-select v-model="queryParams.source" placeholder="来源" clearable style="width: 120px" @change="loadData">
          <el-option label="AI生成" :value="ScriptSourceEnum.AI" />
          <el-option label="人工编写" :value="ScriptSourceEnum.MANUAL" />
        </el-select>
      ...
      >
        ...
        <el-table-column label="来源" width="80" align="center">
          <template #default="{ row }">
            <el-tag :type="scriptSourceTag(row.source)" size="small">
              {{ scriptSourceLabel(row.source) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="状态" width="90" align="center">
          <template #default="{ row }">
            <el-tag :type="scriptStatusTag(row.status)" size="small">
        ...
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              v-if="row.status === ScriptStatusEnum.DRAFT" text type="success" size="small"
...
import ScriptAPI from "@/api/aitc/script";
import type { OptionItem, PageResult } from "@/api/common";
import type { ScriptItem, ScriptQueryParams } from "@/api/aitc/script";
import { ScriptStatusEnum, ScriptSourceEnum } from "@/enums/aitc";
import {
  scriptSourceTag, scriptSourceLabel, scriptStatusTag, scriptStatusLabel,
} from "../constants";
