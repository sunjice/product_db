...
import { ProjectAPI, SpecAPI, SuiteAPI } from "@/api/aitc/index";
import type { OptionItem, PageResult } from "@/api/common";
import type { SpecItem, SpecForm, SpecQueryParams } from "@/api/aitc/types";
...
const specTypeLabels: Record<string, string> = {
  general: "通用规范",
  module_specific: "模块专用规范",
  common_issues: "常见问题",
};

const taskTypeTag = (t: string) => {
  return t === "case_review" ? "success" : t === "core_select" ? "warning" : "primary";
};

const specTypeTag = (t: string) => {
  return t === "general" ? "" : t === "module_specific" ? "warning" : "info";
};
