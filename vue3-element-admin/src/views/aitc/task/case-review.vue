...
import { useCaseReview, FIELD_LABEL_MAP, displayVal, type FieldItem } from "./composables/useCaseReview";
...
const {
  ...
  startEdit, cancelEdit, saveManualEdit: _saveManualEdit, addStepRow, removeStepRow,
  buildFields,
} = useCaseReview();
...
async function submitReview() {
  ...
  try {
    const fields = buildFields(failFields.value.map(f => ({
      ...
      ...f,
      edited_value: f.edited_value !== undefined
        ? (f.action === 'edit_accept' ? (manualValues[f.field_name] as string) : undefined)
        : undefined,
    }));
