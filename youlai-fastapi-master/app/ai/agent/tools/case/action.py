...
class CompleteFieldsArgs(BaseModel):
    """补全用例字段的参数。"""
    case_id: int = Field(..., description="用例 ID")
    field_hint: str | None = Field(None, description="要补全的字段提示，如 preconditions/topo/test_data/all")
...
def make_complete_fields_tool(ctx: ToolContext) -> StructuredTool:
    """补全用例字段。"""

    async def run(case_id: int, field_hint: str | None = None, **kwargs) -> tuple[str, dict | None]:
        params = {"case_id": case_id, "field_hint": field_hint or "all"}
        return await _run_skill("field_complete", params, ctx)

    return StructuredTool(
        name="complete_case_fields",
        description=(
            "根据用例信息，AI 补全用例的前置条件、测试数据、拓扑等字段。返回草稿卡片。"
            "当用户说'补全字段'、'补充前置条件'、'这个用例信息不完整'、'帮完善用例'时调用。"
        ),
        coroutine=run,
        args_schema=CompleteFieldsArgs,
        response_format="content_and_artifact",
    )
