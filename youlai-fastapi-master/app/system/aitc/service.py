"""测试部 AI 助手 — 业务逻辑层：项目/套件/用例 CRUD + Excel 导入 + 提示词/样本/配置/脚本管理。"""
...
from app.system.aitc.models import (
    AiTcProject, AiTcSuite, AiTcCase, AiTcSample,
    AiTcAiConfig, AiTcTask, AiTcTaskItem, AiTcScript, AiTcReviewRecord, AiTcSpec,
)
from app.system.aitc.schemas import (
    AiConfigCreate, AiConfigQuery, AiConfigUpdate, AiConfigVO,
    ...
    SuiteNodeVO, SuiteVO, TaskConfirmReq, TaskVO, TaskItemVO, TaskCreate,
    OptionVO,
    SpecCreate, SpecQuery, SpecUpdate, SpecVO,
)
from app.system.aitc.constants import (
    CaseImportance, CoreSource, ReviewStatus,
    TaskType, TaskStatus, ScriptSource, ScriptStatus, ConfirmStatus, ItemStatus,
    SampleType, SpecType,
)
...
class AiTcService:
    ...
    async def get_suite_children(
        ...
        case_counts: dict[int, int] = {}
        if child_suite_ids:
            # 收集每个子套件及其所有后代套件 ID（tree_path 字段不可靠，改用 parent_id 递归）
        ...
        if suite_id > 0:
            ...
            if direct_cnt > 0:
                case_rows = await self.db.execute(
                    select(AiTcCase.id, AiTcCase.external_id, AiTcCase.name)
                    .where(AiTcCase.suite_id == suite_id, AiTcCase.is_deleted == 0)
                ...
                for cid, ext_id, cname in case_rows:
                    children.append(SuiteNodeVO(
                        id=-cid,
                        label=f"{ext_id or ''} {cname}",
                        name=cname,
                        project_id=proj_id,
                        parent_id=suite_id,
                        node_type="case",
    ...
    async def get_case_page(self, query: CaseQuery) -> PageResult:
        ...
        return PageResult(
            records=[self._case_to_vo(c, suite_map.get(c.suite_id)) for c in cases],
            total=total, pageNum=query.pageNum, pageSize=query.pageSize,
        )
    ...
    async def get_case_by_id(self, case_id: int) -> CaseVO:
        ...
        if c.suite_id:
            ...
            if s:
                suite_name = s.name
        return self._case_to_vo(c, suite_name)
    ...
    async def get_ai_config_page(self, query: AiConfigQuery) -> PageResult:
        conditions = [AiTcAiConfig.is_deleted == 0]
        if query.keywords:
            kw = f"%{query.keywords}%"
            conditions.append(AiTcAiConfig.name.ilike(kw))
        if query.provider:
            conditions.append(AiTcAiConfig.provider == query.provider)
        if query.status is not None:
            conditions.append(AiTcAiConfig.status == query.status)

        stmt = select(AiTcAiConfig).where(*conditions).order_by(AiTcAiConfig.is_default.desc(), AiTcAiConfig.id)
        count_q = select(func.count()).select_from(stmt.subquery())
        total = (await self.db.execute(count_q)).scalar() or 0
        ...
        items = rows.scalars().all()
        return PageResult(
            records=[self._ai_config_to_vo(c) for c in items],
            total=total, pageNum=query.pageNum, pageSize=query.pageSize,
        )

    async def get_ai_config_options(self, scene: str | None = None) -> list[OptionVO]:
        """获取可用的 AI 配置下拉列表。"""
        stmt = select(AiTcAiConfig.id, AiTcAiConfig.name).where(
            AiTcAiConfig.is_deleted == 0, AiTcAiConfig.status == 1
        )
        if scene:
            # 匹配 scenes JSONB 包含指定场景 或 全局默认
            # 简化处理：查全部，Python 侧过滤
            pass
        rows = await self.db.execute(stmt.order_by(AiTcAiConfig.is_default.desc()))
        return [OptionVO(value=r.id, label=r.name) for r in rows]

    async def create_ai_config(self, form: AiConfigCreate) -> AiConfigVO:
        if form.is_default:
            # 清掉其他默认
            await self.db.execute(
                text("UPDATE ai_tc_ai_configs SET is_default = 0 WHERE is_deleted = 0")
            )
        c = AiTcAiConfig(**form.model_dump())
        self.db.add(c)
        await self.db.flush()
        logger.info(f"AiConfig created: {form.name} id={c.id}")
        return self._ai_config_to_vo(c)

    async def update_ai_config(self, form: AiConfigUpdate) -> AiConfigVO:
        result = await self.db.execute(
            select(AiTcAiConfig).where(AiTcAiConfig.id == form.id, AiTcAiConfig.is_deleted == 0)
        )
        c = result.scalar_one_or_none()
        if c is None:
            raise BusinessException(code=ResultCode.DATA_NOT_FOUND, msg="AI配置不存在")
        if form.is_default:
            await self.db.execute(
                text("UPDATE ai_tc_ai_configs SET is_default = 0 WHERE is_deleted = 0 AND id != :id"),
                {"id": form.id},
            )
        c.external_id = form.external_id
        c.name = form.name
        c.provider = form.provider
        c.api_base = form.api_base
        # 前端传回的 api_key 可能是脱敏后的（含 ****），不更新
        if form.api_key and "****" not in form.api_key:
            c.api_key = form.api_key
        c.model = form.model
        c.temperature = form.temperature
        c.max_tokens = form.max_tokens
        c.scenes = form.scenes
        c.is_default = form.is_default
    ...
    async def delete_ai_config(self, ids: str) -> int:
        id_list = [int(x) for x in ids.split(",") if x.strip()]
        if not id_list:
            raise BusinessException(code=ResultCode.PARAM_VALID_FAIL, msg="请选择配置")
        await self.db.execute(
            text("UPDATE ai_tc_ai_configs SET is_deleted = 1 WHERE id = ANY(:ids)"),
            {"ids": id_list},
        )
        return len(id_list)
    ...
    async def get_pending_review_tree(self, project_id: int) -> list[PendingSuiteNodeVO]:
        ...
        # 2. 查出所有有待审核 task_item 的用例及其所属套件
        suite_case_pending: dict[int, set[int]] = {}  # suite_id -> {case_id, ...}
        if suites:
    ...
    async def get_case_review_detail(self, case_id: int) -> CaseReviewDetailVO:
        ...
        case_vo = self._case_to_vo(case, suite_name) if case else None
    ...
    async def get_task_page(self, query) -> PageResult:
        ...
        from app.system.aitc.schemas import TaskVO
        vos = []
        for t in items:
            vos.append(TaskVO(
    ...
    async def resolve_ai_config(self, config_id: int | None, scene: str) -> AiTcAiConfig | None:
        """解析 AI 配置：指定 ID → 全局默认(is_default=1) → 按 scenes 匹配 → 任意启用。"""
        if config_id:
    ...
    async def mark_case_core(self, case_id: int, reason: str) -> None:
        case = await self.db.get(AiTcCase, case_id)
        if case:
            case.is_core = 1
    ...
    async def get_review_records(self, task_id: int) -> list:
        from app.system.aitc.schemas import ReviewRecordVO
        rows = await self.db.execute(
    ...
    async def _load_samples_text(self, sample_ids: list[int]) -> str:
        """根据 ID 加载样本内容并拼接为文本。"""
        rows = await self.db.execute(
    ...
    async def _load_specs_text(self, spec_ids: list[int]) -> str:
        """根据 ID 加载规范内容并拼接为文本。"""
        if not spec_ids:
            return ""
        rows = await self.db.execute(
    ...
    def _case_to_vo(self, c: AiTcCase, suite_name: str = "") -> CaseVO:
        steps = []
        if c.steps:
            steps = [CaseStep(**s) if isinstance(s, dict) else s for s in c.steps]
        return CaseVO(
            id=c.id, project_id=c.project_id, suite_id=c.suite_id,
            suite_name=suite_name, external_id=c.external_id, name=c.name,
            summary=c.summary, preconditions=c.preconditions,
            topo=c.topo, test_data=c.test_data,
            steps=steps, importance=c.importance,
