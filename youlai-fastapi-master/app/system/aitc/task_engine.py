...
所有数据库操作委托给 AiTcService，结果回写委托给 tasks/ 下各任务处理器。
后台执行逻辑已迁移到 tasks/__init__.py 的 execute_task_bg。
"""
...
from app.system.aitc.schemas import (
    ...
    ReviewItemReq, ReviewRecordVO, CaseVO, CaseStep,
)
...
class TaskEngine:
    ...
    @property
    def _svc(self) -> AiTcService:
        """当前请求 session 对应的 service 实例。"""
        return AiTcService(self.db)
    ...
    async def create_task(self, form: TaskCreate, create_by: str = "") -> TaskVO:
        ...
        # 加载 AI 配置
        ai_config = await svc.resolve_ai_config(form.ai_config_id, form.task_type)

        # 解析规范 ID（已记录到任务，调度器执行时再按 ID 加载全文）
        spec_ids, _ = await svc.resolve_specs_text(
    ...
    async def rerun_task(self, task_id: int) -> None:
        ...
        ai_config = await svc.resolve_ai_config(None, task.task_type)
        if ai_config:
    ...
    async def _apply_result(self, svc: AiTcService, task_type: str, item: AiTcTaskItem, ci):
    ...
    async def _apply_script_from_output(
        self, svc: AiTcService, item: AiTcTaskItem, output: dict,
        edited_content: str = "",
    ) -> None:
        """将脚本写入脚本库。"""
